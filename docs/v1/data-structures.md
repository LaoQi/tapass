...
- 时间戳相同时（同一毫秒内的多次写入、或导入的时间戳相同的记录），取**文件中靠后**的一条
  （即最后写入者胜出）；实现必须保证该结果确定，不得依赖哈希/Map 遍历顺序
...
跨平台实现必须与参考实现（`tools/vault`）在**派生密钥、头部字节、数据段字节、密文体字节**上完全一致：

- 测试向量：`tools/vault/testdata/format_v1_vectors.json`（固定 salt/nonce/参数 → 期望的 master key、
  HKDF 各步输出、子密钥、144 字节头部、数据段记录字节、AEAD 密文体字节）
- 独立校验脚本：`python3 docs/v1/verify_format_v1.py`（纯 Python 手写 HKDF 与 XChaCha20-Poly1305，
  按本文件的偏移解析头部与记录，用于校验规范本身可执行）
- 压缩流字节不在校验范围内（见 `crypto-model.md`：只要求解压还原一致）
